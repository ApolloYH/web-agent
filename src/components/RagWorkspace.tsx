...
function PipelinePanel({ collection, busy, onUpdate }: { collection: RagCollection; busy: boolean; onUpdate: (patch: Partial<Pick<RagCollection, 'chunkMethod' | 'pipelineTemplate' | 'pipelineGraph'>>) => Promise<void> }) {
  ...
  return <div className="mx-auto flex h-full min-h-[640px] max-w-[1600px] flex-col"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-[18px] font-semibold text-[#222]">知识流水线</h2><p className="mt-1 text-[10px] text-[#888]">{locked ? '处理配置已锁定；画布仍可缩放、平移和查看。' : '拖动节点与画布，连接端点；滚轮缩放，Delete 删除选中内容。'}</p></div><div className="flex items-center gap-2"><span title="模板在创建知识库时确定" className="inline-flex h-9 items-center rounded-lg bg-[#f3f3f3] px-3 text-[10px] font-medium text-[#666]">模板 · {PIPELINES[collection.pipelineTemplate].label}</span><label className="sr-only" htmlFor="pipeline-add-node">添加节点</label><select id="pipeline-add-node" disabled={locked} value="" onChange={(event) => addNode(event.target.value)} className="h-9 rounded-lg border border-[#d8d8d8] bg-white px-3 text-[10px] text-[#555] outline-none disabled:cursor-not-allowed disabled:bg-[#f3f3f3]"><option value="">＋ 添加节点</option>{NODE_CATALOG.map((item) => <option key={item.type} value={item.type}>{item.label}</option>)}</select><button type="button" disabled={busy || locked || !dirty} onClick={() => { void save(); }} className="h-9 rounded-lg bg-[#155eef] px-4 text-[10px] font-medium text-white hover:bg-[#004eeb] disabled:opacity-35">{locked ? '已锁定' : busy ? '保存中…' : dirty ? '保存流水线' : '已保存'}</button></div></div>
...
function CollectionSettings({ collection, busy, onUpdate, onDelete }: { collection: RagCollection; busy: boolean; onUpdate: (patch: Partial<Pick<RagCollection, 'name' | 'description'>>) => Promise<void>; onDelete: () => Promise<void> }) {
