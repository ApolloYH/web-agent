...
description: 执行威彦达智监业务任务并调用 LangHub 生成成果。用户要求生成管控一张卡、监理通知单、监理日志、监理文书，识别现场隐患，审查施工方案，或对比改造前后单线图时使用；负责结合完整对话和上传附件整理严格输入、检查关键缺项、选择业务类型、调用 run_langhub_task 并保存返回文件。
---
